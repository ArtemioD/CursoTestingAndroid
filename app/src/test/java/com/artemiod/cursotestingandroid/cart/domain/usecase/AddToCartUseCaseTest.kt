...
class AddToCartUseCaseTest {
    ...
    fun insufficient_stock_throws_InsufficientStock() = runTest {
        ...
        assertEquals(
            stockProduct,
            (exception as AppError.Validation.InsufficientStock).availableStock
        )
    }

    @Test
    fun successful_case_adds_item_it_cart() = runTest {
        // Given
        val productId = "id-test-1"
        val product = product {
            withId(productId)
